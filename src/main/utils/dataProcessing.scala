...
object dataProcessing extends Serializable {
  ...
  /**
    ...
    * @return
    */
  def Porcess(dataProcessingMode:String, rdd:RDD[String], spark:SQLContext): DataFrame = {
...
/*          if (x.contains("Failed")) {
  ...
  def STDSjsonProcess(dataProcessingMode:String, rdd:RDD[String], spark:SQLContext): DataFrame = {
    import spark.implicits._
    val kuduDF = rdd.map(_.split(",")).map(p=>stds(p(0),p(1),p(2),p(3),p(4))).toDF()
    return kuduDF
  ...
  def TCdnsProcess(dataProcessingMode:String, rdd:RDD[String], spark:SQLContext): DataFrame = {
    import spark.implicits._
